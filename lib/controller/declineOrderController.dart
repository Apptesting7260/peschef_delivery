...
class DeclineOrderController extends GetxController {
  ...
  final declineOrderData = DeclineOrderModel().obs;
  RxString error = ''.obs;

  void setRxRequestStatus(Status _value) => rxRequestStatus.value = _value;
  void DeclineOrderDataSet(DeclineOrderModel _value) =>
      declineOrderData.value = _value;
  void setError(String _value) => error.value = _value;
  UserPreference userPreference = UserPreference();
  DeclineOrderApi(String id) async {
    SharedPreferences sp = await SharedPreferences.getInstance();
    String token = sp.getString('token') ?? "";
    Map data = {"id": id};
    setRxRequestStatus(Status.LOADING);
    _api.DeclineOrderApi(data, token).then((value) {
      setRxRequestStatus(Status.COMPLETED);
      DeclineOrderDataSet(value);
      if (declineOrderData.value.status == true) {
        homeScreenController.HomeScreenApi();
        Get.back();
      } else {
        Get.back();
        Utils.toastMessage(declineOrderData.value.message.toString());
      }
